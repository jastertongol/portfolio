const mb = document.getElementById('mb'),
    ...
    ml = document.querySelectorAll('.mobile-link'),
    st = document.getElementById('scrollTop'),
    sp = document.querySelector('.scroll-progress');
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        if (mn.classList.contains('active')) closeM();
    }
});
...
document.addEventListener('mousemove', (e) => {
    ...
    shapes.forEach((shape, i) => {
        ...
        shape.style.transform = `translate(${e.clientX * speed}px,${e.clientY * speed}px) rotate(${45 + i * 15}deg)`;
    });
});
        ...
        if (entry.isIntersecting) {
            ...
            if (target.classList.contains('stat-number') && !target.classList.contains('counted')) {
                ...
                const timer = setInterval(() => {
                    current += increment;
                    if (current >= num) {
                        target.textContent = num + (hasPlus ? '+' : '');
                        clearInterval(timer);
...
window.addEventListener('scroll', () => {
    if (!ticking) {
        window.requestAnimationFrame(() => {
            ...
            sp.style.width = (winScroll / height) * 100 + '%';

            st.classList.toggle('visible', scrollY > 500);

            let cur = '';
            document.querySelectorAll('section[id]').forEach(s => {
                const st = s.offsetTop;
                if (scrollY >= (st - 200)) cur = s.getAttribute('id');
            });
...
setTimeout(() => {
    document.querySelectorAll('.code-line').forEach((line, i) => {
        setTimeout(() => line.classList.add('typed'), i * 150);
    });
}, 1200);
